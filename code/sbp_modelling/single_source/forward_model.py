...
from sbp_modelling.single_source.model import SingleSourceModel, v_p_water
from multiprocessing import Pool
from pathlib import Path

try:
    results_path = Path(os.environ['SBP_PATH']) / 'results/single-source'
except KeyError:
    results_path = Path(__file__).parent.resolve() / '../../../results/single-source'
...
def run_run(a_x, a_z, seed):
    ...
    assert f_0 <= v_p_water / (model.d_x * 14)
