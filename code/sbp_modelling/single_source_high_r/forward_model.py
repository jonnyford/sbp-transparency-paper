...
try:
    results_path = Path(os.environ['SBP_PATH']) / 'results/single-source-high-r'
except KeyError:
    results_path = Path(__file__).parent.resolve() / '../../../results/single-source-high-r'

assert results_path.exists()

# Ensure consistent timestep for all models
scratch_model = SingleSourceModel()
d_t = scratch_model.devito_model().critical_dt * 0.9
